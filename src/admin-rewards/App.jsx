...
export default function AdminRewardsApp() {
  ...
  const [summary, setSummary] = useState({ totalMembers: 0, readyCount: 0, nearingCount: 0 });
  ...
  const fetchInsights = async (passToUse) => {
    try {
      setLoading(true);
      setAuthError('');
      const res = await fetch('/api/admin-rewards', {
        ...
        body: JSON.stringify({ password: passToUse || password })
      });
      ...
      console.error('Error fetching admin rewards:', err);
  ...
  return (
    <div className="loyalty-app-wrapper" style={{ maxWidth: '800px', margin: '0 auto', minHeight: '100vh', background: '#F8FAFC' }}>
      {/* Top Header */}
      <header className="loyalty-header" style={{ padding: '16px 24px', background: '#FFFFFF' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <img src="/images/Logo.webp" alt="BAIA Logo" className="loyalty-logo-img" />
          <div>
            <div className="loyalty-logo-title" style={{ fontSize: '1.2rem' }}>BAIA CAFÉ</div>
            <div className="loyalty-logo-sub">BARISTA INSIGHTS &amp; REWARDS</div>
          </div>
        </div>
      ...
      {/* Main Content */}
      <main style={{ padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        {!isAuthenticated ? (
          ...
          <div style={{
            ...
            <h3 style={{ fontFamily: 'Space Grotesk, sans-serif', color: 'var(--loyalty-navy)', marginBottom: '8px', fontSize: '1.25rem' }}>
              Staff &amp; Admin Portal
            </h3>
            <p style={{ fontSize: '0.82rem', color: '#64748B', marginBottom: '24px', lineHeight: '1.4' }}>
              Enter staff passcode to inspect accounts nearing loyalty rewards and active customer journeys.
            </p>
            ...
            <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              {authError && (
                <div style={{ color: '#DC2626', fontSize: '0.8rem', background: '#FEF2F2', padding: '8px', borderRadius: '8px' }}>
                  {authError}
                </div>
              )}
              ...
              >
                {loading ? 'Authenticating...' : 'Access Rewards Roster'}
              </button>
            </form>
          </div>
        ) : (
          /* Authenticated Insights Dashboard */
          <>
