...
export default function AuthModal({ onSuccess, title = "Sign In to Your Loyalty Card", subtitle = "Earn free drinks and tote bags with every coffee order." }) {
          ...
          window.google.accounts.id.initialize({
            ...
            callback: async (response) => {
              try {
                ...
                setErrorMsg(err.message || 'Error signing in with Google ID token.');
              } finally {
  ...
  return (
    <div className="auth-card">
      ...
      {step === 'input-email' && (
        <>
          {/* Native Google One-Tap SDK Button Container (Zero Redirects) */}
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px', width: '100%', marginBottom: '14px' }}>
            <div ref={googleBtnRef} style={{ width: '100%', display: 'flex', justifyContent: 'center', minHeight: '44px' }} />
